...
export function HealthPanel({ isOpen, onClose }: HealthPanelProps) {
  ...
  const runAllTests = async () => {
    ...
    await handleRunTest('llm', () => apiService.testLLM());
    await refetch();
  };
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-semibold text-gray-300">
                  {healthData?.services?.llm?.mode === 'local_ollama' ? 'Ollama (Local)' : 
                   healthData?.services?.llm?.mode === 'fireworks' ? 'LLM (Fireworks)' :
                   'LLM/Model'}
                </h4>
              </div>
              <div className={`w-2 h-2 rounded-full ${
                healthData?.services?.llm?.status === 'available' ? 'bg-emerald-500' : 
                healthData?.services?.llm?.status === 'unavailable' ? 'bg-red-500' : 
                healthData?.services?.llm?.status === 'missing_key' ? 'bg-amber-500' :
                'bg-gray-500'
              }`} />
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-gray-500">Status:</span>
                <span className={
                  healthData?.services?.llm?.status === 'available' ? 'text-emerald-400' :
                  healthData?.services?.llm?.status === 'unavailable' ? 'text-red-400' :
                  healthData?.services?.llm?.status === 'missing_key' ? 'text-amber-400' :
                  'text-gray-400'
                }>
                  {isLoading ? 'Checking...' : 
                   healthData?.services?.llm?.status === 'available' ? 'Available' :
                   healthData?.services?.llm?.status === 'unavailable' ? 'Unavailable' :
                   healthData?.services?.llm?.status === 'missing_key' ? 'Missing API Key' :
                   healthData?.services?.llm?.status || 'Not checked'}
                </span>
              </div>
              {healthData?.services?.llm?.mode && (
                <div className="flex justify-between text-xs">
                  <span className="text-gray-500">Mode:</span>
                  <span className="text-gray-300 font-mono text-[10px]">
                    {healthData.services.llm.mode === 'local_ollama' ? 'Local Ollama' : 
                     healthData.services.llm.mode === 'fireworks' ? 'Fireworks API' :
                     healthData.services.llm.mode}
                  </span>
                </div>
              )}
              {healthData?.services?.llm?.error && (
                <div className="text-[10px] text-red-400 font-mono break-words">
                  Error: {healthData.services.llm.error}
                </div>
              )}
              <button
                onClick={() => handleRunTest('llm', () => apiService.testLLM())}
                className="w-full mt-2 px-2 py-1 text-[10px] font-semibold bg-purple-600 hover:bg-purple-700 text-white rounded transition-colors"
              >
                Test Connection
              </button>
            </div>
          ...
          <div className="p-4 bg-[#0A0B0D] border border-[#1F2937] rounded">
            ...
            <div className="space-y-2">
              {[
                ...
                { name: 'LLM Test', endpoint: '/api/v1/health/test-llm', test: () => apiService.testLLM() },
              ].map((endpoint) => {
                  ...
                  <div key={name} className="p-2 bg-[#14161B] rounded">
                    ...
                    {result.data && (
                      <div className="mt-1 space-y-1">
                        {name === 'LLM Test' && result.data.status === 'success' ? (
                          <>
                            <div className="text-[10px] text-gray-400 space-y-1">
                              <div className="flex justify-between">
                                <span>Mode:</span>
                                <span className="text-emerald-400 font-mono">{result.data.mode}</span>
                              </div>
                              <div className="flex justify-between">
                                <span>Response Time:</span>
                                <span className="text-gray-300 font-mono">{result.data.response_time_ms}ms</span>
                              </div>
                              {result.data.model && (
                                <div className="flex justify-between">
                                  <span>Model:</span>
                                  <span className="text-gray-300 font-mono text-[9px]">{result.data.model}</span>
                                </div>
                              )}
                              {result.data.response && (
                                <div className="mt-2 pt-2 border-t border-gray-700">
                                  <div className="text-[9px] text-gray-500 mb-1">Response:</div>
                                  <div className="text-[10px] text-gray-300 font-mono bg-[#14161B] p-2 rounded">
                                    {result.data.response}
                                  </div>
                                </div>
                              )}
